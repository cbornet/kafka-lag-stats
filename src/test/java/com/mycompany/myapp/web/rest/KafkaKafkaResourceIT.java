...
import com.mycompany.myapp.web.rest.KafkaKafkaResource.SafeKafkaConsumer;
...
class KafkaKafkaResourceIT {
    ...
    private KafkaKafkaResource kafkaResource;
    ...
    void setup() {
        ...
        kafkaResource = new KafkaKafkaResource(kafkaProperties);
    ...
    void createsConsumer() throws Exception {
        restMockMvc.perform(post("/api/kafka-kafka/consumers?name=consumer-create&topic=topic-create&topic=topic-create2&group.id=group-create"))
                .andExpect(status().isOk());

        Field consumersField = ReflectionUtils.findField(KafkaKafkaResource.class, "consumers");
    ...
    void consumesMessages() throws Exception {
        ...
        restMockMvc.perform(get("/api/kafka-kafka/consumers/consumer-consume/records?durationMs=5000"))
                .andExpect(status().isOk())
